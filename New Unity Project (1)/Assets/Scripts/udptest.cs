...
public class udptest : MonoBehaviour
{
    ...
    const int datanum = 3;
    public static float[] f_buf = new float[datanum];
    public static Int32[] i_buf = new Int32[datanum];


    string host = "192.168.11.20";
    int port = 22224;
    ...
    private static void ThreadMethod() //受信？？
    {
        String test;
        ...
        while (true)
        {
            ...
            if (data.Length == datanum * 4 ) // float または Int32 のサイズとして 4
            {
                ...
                */
                //x軸
                f_buf[0] = BitConverter.ToSingle(data, 0 * sizeof(float) );
                test += "angleX: " + f_buf[0].ToString() + ", ";
                //y軸
                f_buf[1] = BitConverter.ToSingle(data, 1 * sizeof(float) );
                test += "angleY: " + f_buf[1].ToString() + ", ";
                //z軸
                f_buf[2] = BitConverter.ToSingle(data, 2 * sizeof(float) );
                test += "angleZ: " + f_buf[2].ToString() + ", ";
    ...
    public void senddataUDP(byte[] d)
    {
        //4byte だけテストで送る
